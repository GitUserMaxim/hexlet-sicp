<p>Бен Битобор, Лиза П. Хакер и Ева Лу Атор спорят о том, каким должен быть результат выражения</p>
<pre><code>(let ((a 1))
  (define (f x)
    (define b (+ a x))
    (define a 5)
    (+ a b))
  (f 10))</code></pre>
<p>Бен говорит, что следует действовать согласно последовательному правилу для 
<code>define</code>
: 
<code>b</code>
 равно 
